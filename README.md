...
Direct touch: the **Direct touch** profile sends absolute finger coordinates from
clicks/drags on the mirrored image. It requires iPhone Accessibility > Zoom enabled
at Full Screen / 1x with the Zoom Controller off. Switching report profiles may
require re-pairing. Exact touch placement on this phone is still a hardware
validation gate. Direct touch does not send hover motion. Relative mouse mode can
trigger the operating system's pointer lift/highlight effects; iMirror has no
custom hover-scaling effect.
