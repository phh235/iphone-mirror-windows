...
static gboolean imirror_stop_callback(gpointer event) {
    if (WaitForSingleObject((HANDLE) event, 0) == WAIT_OBJECT_0) {
...
#ifdef _WIN32
    gmainloop = loop;
    HANDLE imirror_stop_event = NULL;
    guint imirror_stop_watch = 0;
    const char *imirror_event_name = getenv("IMIRROR_STOP_EVENT");
    if (imirror_event_name) {
        imirror_stop_event = OpenEventA(SYNCHRONIZE, FALSE, imirror_event_name);
        if (imirror_stop_event)
            imirror_stop_watch = g_timeout_add(50, imirror_stop_callback, imirror_stop_event);
    }
...
#ifdef _WIN32
    if (imirror_stop_watch) g_source_remove(imirror_stop_watch);
    if (imirror_stop_event) CloseHandle(imirror_stop_event);
    gmainloop = NULL;
#else
...
#ifdef _WIN32
        // iMirror: signal actual disconnect separately from media inactivity.
        // A static iPhone screen may legitimately send no new video frames.
        const char *event_name = getenv("IMIRROR_DISCONNECT_EVENT");
        if (event_name) {
...
extern "C" void video_process (void *cls, raop_ntp_t *ntp, video_decode_struct *data) {
    ...
    if (use_video) {
        ...
        /* A renderer retry must start from the original remote timestamp. */
        const uint64_t remote_time = data->ntp_time_remote;
        do {
            data->ntp_time_remote = remote_time + remote_clock_offset;
